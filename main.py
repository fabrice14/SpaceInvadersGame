...
def startGame():
    global move_delay, move_counter, move_sequence, builder, ship, shield, laserPlayer, counterTime, LEVEL, DIFFICULTY
    move_counter = move_sequence = counterTime = 0
    move_delay = 30
    LEVEL += 1
    if 5 != DIFFICULTY:
        DIFFICULTY -= 5
...
def draw():
    global LEVEL, counterTime, GameState
...
def updateAliens():
    ...
    for element in liste:
        ...
        if element.getActor().type == 1 and randint(0, 2) == 0:
            builder.laser(element.getActor().x, element.getActor().y)
        if element.getActor().y > 670 and ship.getStatus() == 0:
            ship.getActor().status = 1
...
def checkLaserHitAlien():
    global builder, SCORE, laserPlayer
    liste = builder.aliensList.list
    for element in liste:
